...
# LogParserを実行する
& 'C:\Program Files (x86)\Log Parser 2.2\LogParser.exe' -i:EVT -o:CSV -stats:OFF　"SELECT TO_DATE(TimeWritten) AS DATE ,MIN(TimeWritten) AS TIME FROM 'System' WHERE ( TimeWritten BETWEEN TO_LOCALTIME(TIMESTAMP('$start_date 00:00:00', 'yyyy-MM-dd hh:mm:ss')) AND TO_LOCALTIME(TIMESTAMP('$end_date 00:00:00', 'yyyy-MM-dd hh:mm:ss')) )  AND ((EventID = 7001) or (EventID = 6005)) group by DATE" > $dialog.FileName
